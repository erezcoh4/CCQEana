...
public:
    ...
    void        FilterGoodTrajPair (std::vector<pairVertex> & truth_pairs_vector );
    ...
    void      FormPairFromTwoTrajs (PandoraNuTrack ti,PandoraNuTrack tj, TVector3 position);
...
void ub::CosmicTracksAnalyzer::analyze(art::Event const & evt){
    ...
    if (Nparticles){
        ...
        FindTrajPairs( truth_trajectories , truth_pairs );
        AnalyzeVertices( truth_pairs );
        FilterGoodTrajPair( truth_pairs );
...
void ub::CosmicTracksAnalyzer::FindTrajPairs(std::vector<PandoraNuTrack> & traj_vector // tracks to be clustered
                                             ,std::vector<pairVertex> & pairs_vector  // pairs vector to hold the pairs
){
    ...
    for ( size_t i=0; i < traj_vector.size(); i++){
        ...
        for ( size_t j=i+1 ; j < traj_vector.size() ; j++ ){
            ...
            Debug(0,"closest SqDist of i=% & j=%: %",i,j,closest_SqDist_ij);
            if ( closest_SqDist_ij < kMaxInterTrackDistance*kMaxInterTrackDistance ){
                ...
                FormPairFromTwoTrajs( traj_vector[i] , traj_vector[j] , QuasiIntersectionPoint );
                Debug(0,"formed pair from i=% & j=% at %,%,%"
                      ,i,j
                      ,truth_pairs.back().GetPosition().X()
                      ,truth_pairs.back().GetPosition().Y()
                      ,truth_pairs.back().GetPosition().Z());
                
                SearchOtherCloseTrajs( pairs_vector.back() , i , j );
            }
        }
    }
}
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void ub::CosmicTracksAnalyzer::FormPairFromTwoTrajs(PandoraNuTrack ti,PandoraNuTrack tj, TVector3 pos){
...
void ub::CosmicTracksAnalyzer::SearchOtherCloseTrajs( pairVertex & pair , int i , int j ){
    Debug(0 , "SearchOtherCloseTrajs( pairVertex % , i=% , j=% )",pair.GetVertexID(),i,j);
    for ( size_t k=0 ; k < truth_trajectories.size() ; k++ ){
        if ((int)k!=i && (int)k!=j){
            ...
            if( mygeoalgo.SqDist( line_k, VtxPos) < kMaxInterTrackDistance*kMaxInterTrackDistance ){
                ...
                Debug(0,"Adding traj k=% to traj j=% and i=%, and setting position to %,%,%..."
                      ,k,j,i,pair.GetPosition().X(),pair.GetPosition().Y(),pair.GetPosition().Z());
            }
        }
    }
}
...
//    // we want to ask what is the closest distance between their trajectories,
//    // and not between their start or end points
//    bool    FoundCloseTrajectories=false;
//    float   closest_distance_ij;
//    TVector3 vertex_position;
//    
//    // loop over all tracks - to look for partners at close proximity
//    for ( size_t i=0; i < traj_vector.size(); i++){
//        pairVertex pair( run, subrun, event , pairs_vector.size() );
//        pair.AddTrack( traj_vector[i] );
//        FoundCloseTrajectories = false;
//        
//        for ( size_t j=i+1 ; j < traj_vector.size() ; j++ ){
//            if (j!=i){
//                if (!FoundCloseTrajectories){
//                    TVector3 QuasiIntersectionPoint(-1,-1,-1);
//                    closest_distance_ij = traj_vector[i].ClosestLine2LineDistance( traj_vector[j]
//                                                                                  , &QuasiIntersectionPoint);
//                    Debug(0,"closest distance of i=% & j=%: %",i,j,closest_distance_ij);
//                    if ( closest_distance_ij < kMaxInterTrackDistance ){
//                        // if trk-i and trk-j are close enough - we want to keep this vertex
//                        pair.AddTrack( traj_vector[j] );
//                        FoundCloseTrajectories = true;
//                        pair.SetPosition( QuasiIntersectionPoint );
//                        Debug(0,"Adding traj j=% to traj i=%, forming a pair!",j,i);
//                    } // end if keep vertex
//                } // end if !FoundCloseTrajectories
//                else { // (namely if FoundCloseTrajectories==true)
//                    Debug(0,"closest distance of i=% & j=%: %",i,j,closest_distance_ij);
//                    if ( traj_vector[j].ClosestLine2PointDistance(pair.GetPosition()) < kMaxInterTrackDistance ){
//                        Debug(0,"Adding traj j=% to traj i=%, more than a pair...",j,i);
//                        pair.AddTrack( traj_vector[j] );
//                    }
//                }
//            }
//        } // end loop on track-j
//        if (FoundCloseTrajectories) { // plug into vertices list
...
//    // and not between their start or end points
//    bool    FoundCloseTrajectories=false;
//    float   min_closest_distance_ij = kMaxInterTrackDistance, closest_distance_ij;
//    TVector3 vertex_position;
//
//    // loop over all tracks - to look for partners at close proximity
//    for ( size_t i=0; i < traj_vector.size(); i++){
...
//        FoundCloseTrajectories = false;
//
//        art::Ptr<simb::MCParticle> mcparticle_i = traj_mcparticles[i];
//        Debug(0 , "N(trajectory points) in trajectory i=% is %",i,mcparticle_i->NumberTrajectoryPoints ());
//
//        for ( size_t j=i+1 ; j < traj_vector.size() ; j++ ){
//            if (j!=i){
//
//                art::Ptr<simb::MCParticle> mcparticle_j = traj_mcparticles[j];
//                Debug(0 , "N(trajectory points) in trajectory j=% is %",j,mcparticle_j->NumberTrajectoryPoints ());
//
//                if (!FoundCloseTrajectories){
//                    for ( int p_i_step=0; p_i_step < (int)mcparticle_i->NumberTrajectoryPoints(); p_i_step++) {
//
//                        TVector3 p_i_step_pos = mcparticle_i->Position( p_i_step ).Vect();
//
//                        for ( int p_j_step=0; p_j_step < (int)mcparticle_j->NumberTrajectoryPoints (); p_j_step++) {
//
//                            TVector3 p_j_step_pos = mcparticle_j->Position( p_j_step ).Vect();
//                            closest_distance_ij = (p_j_step_pos-p_i_step_pos).Mag();
//                            if (  closest_distance_ij < min_closest_distance_ij ){
//                                min_closest_distance_ij = closest_distance_ij;
//                                pair.SetPosition( 0.5*(p_j_step_pos+p_i_step_pos) );
//                            }
//                        } // end loop on steps of particle-j
//                    } // end loop on steps of particle-i
//                    if ( min_closest_distance_ij < kMaxInterTrackDistance ) {
//                        pair.AddTrack( traj_vector[j] );
//                        FoundCloseTrajectories = true;
//                        Debug(0 , "Added trajectory j=% to trajectory i=% - pair!",j,i);
//                    }
//                } // end FoundCloseTrajectories if statement
//
//                //                        TVector3 QuasiIntersectionPoint(-1,-1,-1);
//                //                        closest_distance_ij = traj_vector[i].ClosestLine2LineDistance( traj_vector[j]
//                //                                                                                      , &QuasiIntersectionPoint);
...
//                //                    } // end if !FoundCloseTrajectories
//                else { // (namely if FoundCloseTrajectories==true)
//                    min_closest_distance_ij = kMaxInterTrackDistance;
//                    for ( int p_i_step=0; p_i_step < (int)mcparticle_i->NumberTrajectoryPoints(); p_i_step++) {
//
//                        TVector3 p_i_step_pos = mcparticle_i->Position( p_i_step ).Vect();
//                        for ( int p_j_step=0; p_j_step < (int)mcparticle_j->NumberTrajectoryPoints (); p_j_step++) {
...
//                    if (  min_closest_distance_ij < kMaxInterTrackDistance ){
//                        pair.AddTrack( traj_vector[j] );
//                        Debug(0,"Added trajectory j=% to trajectory i=% - more than a pair!",j,i);
//                    }
//                } // end {FoundCloseTrajectories==true} else
//            } // end j!=i if statement
//        } // end loop on track-j
//
//
//        if (FoundCloseTrajectories) { // plug into vertices list
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
void ub::CosmicTracksAnalyzer::FilterGoodTrajPair(std::vector<pairVertex> & truth_pairs_vector ){
    
    std::vector<pairVertex> tmp_pairs = truth_pairs_vector;
    truth_pairs_vector.clear();
    Debug(2 , "ub::CosmicTracksAnalyzer::FilterGoodTrajPair()");
    for (auto & pair:tmp_pairs) {
        
        if (debug>2) {
            cout << "filtering Trajectories-pair "<< pair.GetVertexID() << " with tracks: ";
        ...
        if (pair.GetNtracks() == 2){
            ...
            truth_pairs_vector.push_back( pair );
        } else {
...
void ub::CosmicTracksAnalyzer::StreamTrajectoryPairsToCSV(){
    ...
    for (auto pair : truth_pairs) {
        ...
        for ( auto d : pair.Get_distances_ij()){
            truth_pairs_file << d ;
        }

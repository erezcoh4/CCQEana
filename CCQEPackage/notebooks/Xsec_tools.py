...
def extract_Xsec_full_chain(extra_name='',debug=0,eff_cutoff=0.025
    ...
    if (('W(corr. phi~0)' not in selected_beam_on.columns) or ('W(corr. phi~0)' not in selected_CC1p.columns)):#{
        ...
        Xsec_dict = get_phi_Xsecs( do_corr_phi_0=False, debug=debug, evtwgt_name=evtwgt_name,
        ...
        correction_arrays['mu']=[]; correction_arrays['p']=[]; correction['mu']=1; correction['p']=1; current_correction=100
        while np.abs(current_correction-1)>0.0001:#{
            for particle,var in zip(['mu','p'],['reco_Pmu_mcs_phi','reco_Pp_phi']):#{
                ...
                for sam in [selected_beam_on,selected_beam_off,selected_CC1p,selected_overlay_concat]:#{
                    ...
                    Xsec_dict = get_phi_Xsecs( do_corr_phi_0=True, debug=debug, evtwgt_name=evtwgt_name,
                                              selected_beam_on=selected_beam_on,selected_beam_off=selected_beam_off,
                                              selected_overlay_concat=selected_overlay_concat,selected_CC1p=selected_CC1p)
                #}
            #}
...
# ----------------------------------------------------------
# Oct-08, 2018
def get_phi_Xsecs(do_corr_phi_0=False, debug=0, evtwgt_name='',
...
# ----------------------------------------------------------
# Oct-08, 2018
def get_Xsec_variable(debug=0,evtwgt_name='',
...
def draw_Xsec_variable(debug=0,evtwgt_name='',y_scale=1,
    ...
    for Xsec_ctu_title,remove_ctu_bin in zip(Xsec_ctu_titles,remove_ctu_bools):
        Xsec_dicts[Xsec_ctu_title] = get_Xsec_variable(debug=debug,evtwgt_name=evtwgt_name,
...
def get_Xsec_label(vlabel=None,units=None,power_factor=38):
    ...
    if vlabel is None:      return r'$\times '+factor_str+' \mathrm{cm}^{2}$'
    prefix = r'$\frac{d\sigma}{d'+vlabel+'}$ '
    if units is not None:   suffix = r'$\left['+factor_str+r' \frac{\mathrm{cm}^{2}}{%s} \right]$'%units
    else:                   suffix = r'$\left['+factor_str+r' \mathrm{cm}^{2}\right]$'
...
def compute_eff_weights(beam_on=None,beam_off=None,evtwgt_name=''
    ...
    for i_P in range(N1):#{
        ...
        for i_cos_theta in range(N2):#{
            ...
            for i_phi in range(N3):#{
                ...
                gen_CC1p_in_kin_cuts = generated_CC1p[(np.abs(generated_CC1p['theta_12']-90)<delta_theta_12)
                                                            &(generated_CC1p['Pt']<Pt_max)
                                                            &(np.abs(np.abs(generated_CC1p['delta_phi'])-180.)<delta_Delta_phi)]

                N['gen. in kin. cuts'],List['gen. in kin. cuts'] = len_sam_in_3d_bin(gen_CC1p_in_kin_cuts,
                                                                                     true_1,Pmin,Pmax,
                                                                                     true_2,cos_theta_min,cos_theta_max,
                                                                                     true_3,phi_min,phi_max,
                                                                                     evtwgt_name=evtwgt_name)
...
# ----------------------------------------------------------
# Sep-02, 2018 (last edited Oct-11)
def len_sam_in_3d_bin(sam,
    ...
    if evtwgt_name=='': return np.float(len(list_of_indices)),list_of_indices
    else:               return np.sum(sam.loc[list_of_indices][evtwgt_name]),list_of_indices
#}
# ----------------------------------------------------------
...
def load_mc_and_data(extra_name='',debug=0
    ...
    #}
        ...
        Nevents['overlay evts'] = np.sum(summary.Nevents)
...
## deprecated: using weights to compute cross-section
## ----------------------------------------------------------
## Aug-27, 2018
